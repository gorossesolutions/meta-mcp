...
**Régression trouvée en vérifiant `0011` contre le job de sync réel** (`db/migrations/0012_fix_svc_sync_auth_short_circuit.sql`) : juste après avoir appliqué `0011`, `svc_sync` s'est mis à échouer avec `permission denied for schema auth` sur de simples écritures. Cause : `app.has_client_access()` s'écrit `current_user = 'svc_sync' OR EXISTS (... app.current_user_id() ...)`, et le `OR` du SQL standard **ne garantit pas** de court-circuiter avant une sous-requête — Postgres a quand même évalué `app.current_user_id()` (donc `auth.user_id()`) alors même que `svc_sync` aurait dû suffire à trancher. `svc_sync` n'a (et ne doit pas avoir) de droit sur le schéma `auth` de Neon. Corrigé en réécrivant `app.current_user_id()` en PL/pgSQL avec un `IF current_user = 'svc_sync' THEN RETURN NULL; END IF;` explicite avant tout appel à `auth.user_id()` — le contrôle de flux procédural garantit ce que le `OR` SQL ne garantissait pas. Un seul point de correction (la fonction), pas besoin de toucher chaque policy. Revérifié en conditions réelles après coup : dry-run et vrai sync repassent au vert, comptes de lignes inchangés (aucun doublon).

**Même symptôme, cette fois pour `authenticated`** (`db/migrations/0013_grant_auth_schema_usage.sql`) : premier test live du dashboard (Étape 5) contre un utilisateur avec un vrai `user_clients`, même erreur `permission denied for schema auth` — cette fois-ci parce qu'`authenticated` (contrairement à `svc_sync`) a réellement besoin d'appeler `auth.user_id()`. `GRANT EXECUTE ON FUNCTION auth.user_id() TO authenticated` était déjà en place (fait par Neon à la création du rôle), mais `GRANT USAGE ON SCHEMA auth TO authenticated` manquait — nécessaire en plus de l'`EXECUTE` sur la fonction elle-même dès que la fonction appelante (`app.current_user_id()`) n'est pas `SECURITY DEFINER`. Tenté d'abord en ajoutant ce `GRANT USAGE` : Postgres répond `GRANT` sans erreur, mais `pg_namespace.nspacl` du schéma `auth` (propriété de `cloud_admin`, le rôle interne de Neon) ne change jamais, re-vérifié immédiatement dans la même session — ce chemin ne fonctionne pas sur ce projet, cause exacte non identifiée (ACL géré/reset en dehors du SQL normal côté Neon, probablement). `neondb_owner`, lui, a un accès réel et fonctionnel au schéma `auth` (confirmé : `SELECT auth.user_id()` réussit, retourne `NULL` hors contexte JWT plutôt que d'échouer). Fix retenu : `ALTER FUNCTION app.current_user_id() SECURITY DEFINER SET search_path = auth, pg_catalog` — la fonction s'exécute désormais avec les droits de son propriétaire (`neondb_owner`) quel que soit l'appelant, contournant le problème d'ACL sur `auth` plutôt que d'essayer de le résoudre. `search_path` épinglé explicitement, protection standard contre le détournement de `SECURITY DEFINER` par manipulation du search_path. Revérifié en conditions réelles depuis le navigateur (voir résumé de session Étape 5) : lecture RLS-scopée fonctionnelle, écriture hors `creative_angles` toujours rejetée en 403 sur `clients`/`campaigns_snapshot`/`insights_daily`.
